...
select c.cus_gender, count(*) from customer c join `order` o on o.cus_id = c.cus_id where o.ord_amount >= 3000 group by c.cus_gender;

-- Question 4:
/*Display all the orders along with the product name ordered by a customer having
Customer_Id=2.*/
select pro_name, o.* from `order` o, product_details pd, product p where o.cus_id = 2 and o.prod_id = p.pro_id and p.pro_id = pd.pro_id;
...
select category.* from `order` join  product_details on `order`.prod_id = product_details.prod_id inner join product on product.pro_id = product_details.pro_id inner join category on  category.cat_id = product.cat_id where `order`.ord_amount = (select min(ord_amount) from `order`);

-- Question 7:
/*Display the Id and Name of the Product ordered after “2021-10-05”.*/
select p.pro_id, p.pro_name from `order` o join product_details pd on pd.prod_id = o.prod_id join product p on p.pro_id = pd.pro_id where o.ord_date > "2021-10-05";
...
select s.supp_id, s.supp_name, c.cus_name,r.rat_ratstars from supplier s join rating r on s.supp_id = r.supp_id join customer c on  r.cus_id = c.cus_id  order by r.rat_ratstars desc limit 3 
